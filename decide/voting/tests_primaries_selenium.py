...
class TestAddPoliticalParty(unittest.TestCase):
    ...
    def test_create_presidential_primary_voting_and_make_president(self):
        ...
        #We added all the necessary fields to be able to save the voting
        self.driver.find_element_by_id('id_name').send_keys('Primmarias PP')

        
        select = Select(self.driver.find_element_by_id('id_question'))
        select.select_by_visible_text("¿quien quieres que sea el presidente del partido popular?")

        select = Select(self.driver.find_element_by_id('id_tipe'))
        select.select_by_visible_text("Presidential primaries")
        ...
        self.driver.find_element_by_link_text("partido popular").click()
        
        president=self.driver.find_elements_by_xpath("//*[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZЙ', 'abcdefghijklmnopqrstuvwxyzй'), 'user2')]") 
        self.assertTrue(president != [])
